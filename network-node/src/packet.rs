...
use crate::utils::util::{
    self, get_localnet_location, is_neighbor_node_in_localnet, is_same_localnet,
};
...
impl Packet {
    ...
    pub fn make_check_connection_packet(source: Id) -> Packet {
        Self::make_broadcast_packet(0, Header::HCheckConnection, source).unwrap()
    }

    pub fn make_broadcast_packet(
        packet_id: PacketId,
        header: Header,
        source: Id,
    ) -> Result<Packet> {
        if !header.is_only_head() {
            return Err(anyhow!("Header is not only head: {:?}", header));
        }
        let packet = Self::new(
            packet_id,
            header,
    ...
    pub fn make_confirm_coordinate_packet_by_confirmed_node(
        source: Id,
        destination: Id,
        this_coordinate: Coordinate,
        this_global_location: LocalNetworkLocation,
    ) -> Option<Packet> {
        // if the source node is confirmed and the packet is sent to other network, coordinate is only one, which is the coordiate of the source node.

        if !is_same_localnet(source, destination) {
            return Self::make_confirm_coordinate_packet_to_different_localnet_by_confirmed_node(
                source,
                this_coordinate,
            );
        }
        ...
        let coordinates =
            Self::make_localnet_coordinates(source, this_coordinate, this_global_location);

        messages.push(0xFF);

        for (from_id, _, (x, y)) in coordinates {
            messages.append(&mut from_id.to_be_bytes().to_vec());
            messages.append(&mut x.to_be_bytes().to_vec());
            messages.append(&mut y.to_be_bytes().to_vec());
        }
    ...
    fn make_localnet_coordinates(
        source: Id,
        this_coordinate: Coordinate,
        this_global_location: LocalNetworkLocation,
    ) -> Vec<(Id, Id, Coordinate)> {
        let this_local_location = get_localnet_location(source);
        let mut coordinates = Vec::new();
        let diff = this_local_location - this_global_location;
        let base_coordinate = [
        ...
        for (global_location, coordinate) in base_coordinate {
            ...
            let raw_location: Id = local_location.into();

            let id = raw_localnet_id | raw_location | raw_is_root;
            coordinates.push((id, id, coordinate));
        }
        coordinates
    }

    pub fn make_confirm_coordinate_packet(
        source: Id,
        destination: Id,
        coordinates: &Vec<(Id, Id, Coordinate)>,
    ) -> Option<Packet> {
        // if the source node is confirmed and the packet is sent to other network, coordinate is only one, which is the coordiate of the source node.

        if !is_same_localnet(source, destination) || coordinates.len() == 0 {
            // not confirmed node and not have coordinates
            return None;
        }
        ...
        messages.push(0);
        for (from_id, coordinate_id, (x, y)) in coordinates {
            if from_id != coordinate_id {
                // it is from other localnet
                continue;
            }
            messages.append(&mut from_id.to_be_bytes().to_vec());
            messages.append(&mut x.to_be_bytes().to_vec());
    ...
    fn make_confirm_coordinate_packet_to_different_localnet_by_confirmed_node(
        source: Id,
        this_coordinate: Coordinate,
    ) -> Option<Packet> {
        let mut messages = Vec::new();
...
mod test {
    ...
    fn test_request_confirmed_coordinate_packet() {
        ...
        let coordinate = (1, 2);
        let packet =
            Packet::make_confirm_coordinate_packet_to_different_localnet_by_confirmed_node(
                3, coordinate,
            )
            .unwrap();
